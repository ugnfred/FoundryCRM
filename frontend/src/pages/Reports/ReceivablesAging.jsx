...
function AgingBar({ row }) {
  const total = row.total || 1
  const bucketColors = ['bg-green-400', 'bg-yellow-400', 'bg-orange-400', 'bg-red-400', 'bg-red-700']
  return (
    <div className="flex h-2 rounded overflow-hidden w-full">
      {BUCKETS.map((b, i) => {
        const pct = ((row[b] || 0) / total) * 100
        return pct > 0 ? <div key={b} className={bucketColors[i]} style={{ width: `${pct}%` }} title={`${b}: ₹${fmt(row[b])}`} /> : null
      })}
    </div>
  )
}
...
export default function ReceivablesAging() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['receivables-aging', queryParams],
    queryFn: () => reportsApi.receivablesAging(queryParams),
    enabled: !!queryParams,
  })
  ...
  const handleExcel = async () => {
    setFetching(true)
    try {
      const blob = await reportsApi.receivablesAgingXlsx({ as_of: asOf })
      downloadBlob(blob, `Receivables_Aging_${asOf}.xlsx`)
    } finally {
      setFetching(false)
    }
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">Outstanding customer balances by age bucket (based on due date)</p>
      </div>
      ...
      {data && rows.length === 0 && (
        <div className="text-center py-16 text-slate-400">
          <p className="text-lg">No outstanding receivables as of {data.as_of}</p>
        </div>
      )}
        ...
        <>
          ...
          <div className="bg-white border rounded-lg overflow-hidden">
            <div className="bg-blue-600 text-white px-4 py-3 flex items-center justify-between">
              <h3 className="font-semibold text-sm">Customer-wise Aging — As of {data.as_of}</h3>
              <span className="text-xs bg-white/20 px-2 py-1 rounded">{rows.length} customers</span>
            </div>
              ...
              <table className="w-full text-sm">
                <thead className="bg-slate-100">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-left font-medium w-24">Bar</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map((row, i) => (
                    <tr key={i} className="border-t hover:bg-slate-50">
                      ...
                      <td className="px-3 py-2"><AgingBar row={row} /></td>
                    </tr>
                  ))}
                </tbody>
                <tfoot className="border-t-2 border-slate-300 bg-blue-50 font-semibold">
                  <tr>
                    <td className="px-3 py-3" colSpan={2}>TOTAL ({rows.length} customers)</td>
                    {BUCKETS.map((b) => (
