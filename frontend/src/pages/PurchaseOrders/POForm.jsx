...
export default function POForm({ open, onClose, existing }) {
  ...
  const { register, control, handleSubmit, setValue, watch } = useForm({
    defaultValues: existing
      ? { ...existing, items: existing.po_items ?? existing.items ?? [emptyItem] }
      : { company_id: '', date: new Date().toISOString().slice(0, 10), delivery_date: '', status: 'draft', items: [emptyItem] },
  })
  ...
  const mutation = useMutation({
    mutationFn: (d) => existing ? purchaseOrdersApi.update(existing.id, d) : purchaseOrdersApi.create(d),
    onSuccess: () => { toast({ title: 'Saved' }); qc.invalidateQueries(['purchase-orders']); onClose() },
    onError: (e) => toast({ title: 'Error', description: e.message, variant: 'destructive' }),
  })

  function fillFromProduct(index, productId) {
    const p = products.find((x) => x.id === productId); if (!p) return
    setValue(`items.${index}.product_id`, p.id); setValue(`items.${index}.description`, p.name)
    setValue(`items.${index}.hsn_code`, p.hsn_code); setValue(`items.${index}.uom`, p.uom)
    setValue(`items.${index}.rate`, p.base_rate); setValue(`items.${index}.gst_rate`, p.gst_rate)
  }

  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader><DialogTitle>{existing ? 'Edit Purchase Order' : 'New Purchase Order'}</DialogTitle></DialogHeader>
        <DialogBody>
          <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-5">
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
              ...
              <div className="space-y-1.5"><Label>Delivery Date</Label><Input type="date" {...register('delivery_date')} /></div>
            </div>
            <div>
