...
export default function SOForm({ open, onClose, existing }) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { toast({ title: 'Saved', description: 'Sales order saved.' }); qc.invalidateQueries(['orders']); onClose() },
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader><DialogTitle>{existing ? 'Edit Sales Order' : 'New Sales Order'}</DialogTitle></DialogHeader>
        <DialogBody>
          <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-5">
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
              ...
              <div className="col-span-2 space-y-1.5"><Label>Buyer PO Reference</Label><Input placeholder="Customer's PO number" {...register('po_reference')} /></div>
            </div>
            <div>
              ...
              <div className="rounded-lg border overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 border-b">
                    <tr>{['Product', 'Description', 'HSN', 'UOM', 'Qty', 'Rate', 'GST%', 'Amount', ''].map((h) => (
              ...
              <div className="mt-3 flex justify-end text-sm space-y-1 text-right">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button type="submit" disabled={mutation.isPending}>{mutation.isPending ? 'Saving…' : 'Save Order'}</Button>
            </div>
          </form>
        </DialogBody>
      </DialogContent>
